package main

import (
	"encoding/base64"
	"fmt"

	"netacom.vn/netachat/OTP/usecase"
	"netacom.vn/netachat/domain"
)

func main() {
	//const HOST = "sms.8x77.vn"
	const HOST = "167.71.218.111"

	var url = fmt.Sprintf("http://%s:8077/mt-services/MTService", HOST)

	config := domain.ConfigMTSendOTP{
		URL:      url,
		USERNAME: "netacom",
		PASSWORD: "netacom@!NET789",
	}

	req := domain.SendMessageRequest{
		Phone: "84987677109",
		Text:  base64.StdEncoding.EncodeToString([]byte("OTP cua may ban la")),
	}

	httpReq, err := usecase.GenerateSOAPRequestMTOTP(&config, &req)
	if err != nil {
		fmt.Println("Some problem occurred in request generation")
	}

	_, err = usecase.SoapCallMTOTP(httpReq)
	if err != nil {
		fmt.Println("Problem occurred in making a SOAP call")
	}

	//fmt.Printf("SOAP response: %v\n", response)
}
